...
public class Lab6 extends JPanel implements GLEventListener {
	...
	private void triangularPrism(GL2 gl2) {
		...
		gl2.glTexCoord2d(1/3.0, 0);
		gl2.glVertex3d(-t, -0.5, -0.25);
		gl2.glTexCoord2d(1/3.0, 1);
		gl2.glVertex3d(-t, 0.5, -0.25);
		gl2.glTexCoord2d(0, 1);
		gl2.glVertex3d(t, 0.5, -0.25);
		...
		gl2.glTexCoord2d(1 / 3.0, 0);
		gl2.glVertex3d(-t, -0.5, -0.25);
		gl2.glTexCoord2d(2 / 3.0, 0);
		gl2.glVertex3d(0, -0.5, 0.5);
		gl2.glTexCoord2d(2 / 3.0, 1);
		gl2.glVertex3d(0, 0.5, 0.5);
		gl2.glEnd();
	...
	private Texture textureFromResource(String resourceName) {
		...
		// Will be set to true if context needs to be made current.
	...
	private Texture textureFromPainting() {
		...
		GL2 gl2 = context.getGL().getGL2();
		try {
			// Get the BufferedImage from the paint panel
	...
	public void display(GLAutoDrawable drawable) {
		...
		if (currentTexture != null) {
			// Enable texture if there's a current texture to apply
		...
		gl2.glMatrixMode(GL2.GL_TEXTURE); // Set the matrix mode to texture matrix
		gl2.glLoadIdentity();

		// Apply horizontal and vertical scaling for texture repeat
